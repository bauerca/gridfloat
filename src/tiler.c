...
void print_usage(void) {
    fprintf(stdout,
        ...
        "  -s:  Dimensions of smallest tile. Attach a suffix 'm' or\n"
        "       'km' for units.\n"
...
int main(int argc, char *argv[]) {
    ...
    /* Extraction grid */
    gf_grid tile_grid;
    double *b_view[4] = {&tile_grid.left, &tile_grid.right, &tile_grid.bottom, &tile_grid.top};
    int *res_view[2] = {&tile_grid.nx, &tile_grid.ny};
	...
	while (n >= 0) {
        ...
        case 'R':
            ...
            if (count > 2) {
                ...
                tile_grid.ny = tile_grid.nx;
            }
            break;
        case 'l':
            tile_grid.left = atof(optarg);
            break;
        case 'r':
            tile_grid.right = atof(optarg);
            break;
        case 'b':
            tile_grid.bottom = atof(optarg);
            break;
        case 't':
            tile_grid.top = atof(optarg);
            break;
        case 'B':
            count = 0;
            while (optarg != NULL) {
    ...
    if (from_point) {
        gf_init_grid_point(&tile_grid, latlng[0], latlng[1], wh[0], wh[1], tile_grid.ny, tile_grid.nx);
    } else {
        /* Just calculate dx, dy. */
        gf_init_grid_bounds(&tile_grid, tile_grid.left, tile_grid.right, tile_grid.bottom, tile_grid.top, tile_grid.ny, tile_grid.nx);
    }
    ...
    if (info) {
        ...
        if (len > 4 && !strcmp(savename + len - 4, ".png")) {
            ...
            n_sun[2] = sin(polar);
            gf_relief_shade(&gf, &tile_grid, n_sun, savename);
        } else if (len > 4 && !strcmp(savename + len - 4, ".stl")) {
            data = (gf_float *)malloc(tile_grid.nx * tile_grid.ny * sizeof(gf_float));
            gf_bilinear_interpolate(&gf, &tile_grid, data);
            gf_save_stl(&tile_grid, data, savename);
            free(data);
        } else {
            data = (gf_float *)malloc(tile_grid.nx * tile_grid.ny * sizeof(gf_float));
            gf_bilinear_interpolate(&gf, &tile_grid, data);
            gf_save(&tile_grid, data, savename);
            free(data);
        }
